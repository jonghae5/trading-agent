...
class FredService:
    ...
    async def get_gdp_data(self) -> Dict[str, Any]:
        ...
        gdp_indicators = ["GDP", "GDPC1", "GDPDEF"]
        
        tasks = []
        for indicator in gdp_indicators:
            series_id = self.ECONOMIC_INDICATORS[indicator]
            task = self.get_series_observations(series_id, limit=20)
            tasks.append((indicator, task))
        ...
        for indicator, task in tasks:
            try:
                ...
                logger.error(f"Error fetching GDP indicator {indicator}: {e}")
                results[indicator] = []
    ...
    async def get_employment_data(self) -> Dict[str, Any]:
        ...
        employment_indicators = ["UNRATE", "CIVPART", "PAYEMS", "EMRATIO", "ICSA"]
        
        tasks = []
        for indicator in employment_indicators:
            series_id = self.ECONOMIC_INDICATORS[indicator]
            task = self.get_series_observations(series_id, limit=50)
            tasks.append((indicator, task))
        ...
        for indicator, task in tasks:
            try:
                ...
                logger.error(f"Error fetching employment indicator {indicator}: {e}")
                results[indicator] = []
    ...
    async def get_inflation_data(self) -> Dict[str, Any]:
        ...
        tasks = []
        for indicator in inflation_indicators:
            series_id = self.ECONOMIC_INDICATORS[indicator]
            task = self.get_series_observations(series_id, limit=50)
            tasks.append((indicator, task))
        ...
        for indicator, task in tasks:
            try:
                ...
                logger.error(f"Error fetching inflation indicator {indicator}: {e}")
                results[indicator] = []
        
        return results
    
    async def get_interest_rates_data(self) -> Dict[str, Any]:
        """Get comprehensive interest rates data."""
        rates_indicators = ["FEDFUNDS", "DGS10", "DGS2", "DGS3MO", "DGS30", "T10Y2Y"]
        
        tasks = []
        for indicator in rates_indicators:
            series_id = self.ECONOMIC_INDICATORS[indicator]
            task = self.get_series_observations(series_id, limit=100)
            tasks.append((indicator, task))
        ...
        for indicator, task in tasks:
            try:
                ...
                logger.error(f"Error fetching rates indicator {indicator}: {e}")
                results[indicator] = []
    ...
    async def get_housing_data(self) -> Dict[str, Any]:
        ...
        housing_indicators = ["HOUST", "MORTGAGE30US", "CSUSHPISA"]
        ...
        for indicator, task in tasks:
            try:
                ...
                logger.error(f"Error fetching housing indicator {indicator}: {e}")
                results[indicator] = []
    ...
    async def get_economic_summary(self) -> Dict[str, Any]:
        ...
        for indicator in key_indicators:
            try:
                series_id = self.ECONOMIC_INDICATORS[indicator]
                latest = await self.get_latest_value(series_id)
                ...
                logger.error(f"Error fetching summary for {indicator}: {e}")
