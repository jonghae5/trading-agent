...
export const economicApi = {
  /**
   * Get economic summary with latest values
   */
  async getSummary(): Promise<EconomicSummary> {
    const response = await apiClient.get<{ data: EconomicSummary }>(
  ...
  async getSeriesData(params: {
    ...
    const url = queryParams.toString()
      ? `/api/v1/economic/series/${params.seriesId}?${queryParams.toString()}`
      : `/api/v1/economic/series/${params.seriesId}`

    const response = await apiClient.get<{ data: SeriesDataResponse }>(url)
    return response.data
  },

  /**
   * Get GDP-related indicators
   */
  async getGdpIndicators(): Promise<Record<string, EconomicObservation[]>> {
    const response = await apiClient.get<{
      data: Record<string, EconomicObservation[]>
    }>('/api/v1/economic/gdp')
    return response.data
  },
   ...
   */
  async getEmploymentIndicators(): Promise<
    Record<string, EconomicObservation[]>
  > {
    const response = await apiClient.get<{
      data: Record<string, EconomicObservation[]>
    }>('/api/v1/economic/employment')
    return response.data
  },
   ...
   */
  async getInflationIndicators(): Promise<
    Record<string, EconomicObservation[]>
  > {
    const response = await apiClient.get<{
      data: Record<string, EconomicObservation[]>
    }>('/api/v1/economic/inflation')
    return response.data
  },
   ...
   */
  async getInterestRates(): Promise<Record<string, EconomicObservation[]>> {
    const response = await apiClient.get<{
      data: Record<string, EconomicObservation[]>
    }>('/api/v1/economic/interest-rates')
    return response.data
  },
   ...
   */
  async getHousingIndicators(): Promise<Record<string, EconomicObservation[]>> {
    const response = await apiClient.get<{
      data: Record<string, EconomicObservation[]>
    }>('/api/v1/economic/housing')
    return response.data
  },
   ...
   */
  async searchSeries(params: { query: string; limit?: number }): Promise<{
    query: string
    count: number
    series: any[]
  }> {
    ...
    const response = await apiClient.get<{ data: any }>(
      `/api/v1/economic/search?${queryParams.toString()}`
    )
    return response.data
  },
   ...
   */
  async getMultipleIndicators(params: {
    indicators: string[]
    startDate?: string
    endDate?: string
  }): Promise<Record<string, EconomicObservation[]>> {
    const queryParams = new URLSearchParams({
      indicators: params.indicators.join(',')
    })
    ...
    const response = await apiClient.get<{
      data: Record<string, EconomicObservation[]>
    }>(`/api/v1/economic/indicators?${queryParams.toString()}`)
    return response.data
  }
}
